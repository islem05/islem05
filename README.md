# 👋 Hi, I'm Ado

![Typing SVG](https://readme-typing-svg.herokuapp.com?size=25&duration=3000&color=00F7FF&center=true&vCenter=true&width=700&lines=Backend+Developer;Data+%2F+AI+Enthusiast;Master's+Student+in+Networks;and+Systems+Engineering;Building+Real+World+Projects)

---
## 🚀 About Me

🎓 Master's student in Networks and Systems Engineering  
💻 Aspiring Backend / Data / AI Developer  

- 🔧 Strong background in Java, Python, SQL, C#  
- 📊 Currently focused on Data Analysis & AI projects  
- 🌐 Learning Backend Development (APIs, system design)  
- 🧠 I learn best by building real-world projects  

### 🖧 Networking & Systems Skills
- 🖥️ Linux (Ubuntu, command line, basic administration)  
- 🌐 Networking fundamentals (TCP/IP, HTTP, DNS)  
- 🔌 Socket programming & real-time communication  
- ☁️ Basic cloud & virtualization (VirtualBox, Docker)  
- 🔐 Intro to system security & architecture  

---

## 🛠️ Tech Stack

![My Skills](https://skillicons.dev/icons?i=java,python,cs,js,ts,docker,linux,git,nodejs,spring)

---

## 📌 Featured Projects

### 🔹 GoogleBombing

🧠 Python-based project exploring search engine manipulation techniques and keyword ranking behavior
Tech: Python
👉 https://github.com/islem05/GoogleBombing

---

### 🔹 Backend API

🌐 REST API with authentication and database integration
**Tech:** Node.js / Spring Boot, JWT, SQL
👉 https://github.com/YOUR_USERNAME/backend-api

---

### 🔹 P2P File Sharing Application

🌐 Real-time peer-to-peer file sharing system using WebRTC with a signaling server
Tech Stack:

Frontend (Client):

React.js (Vite)

Tailwind CSS

Socket.io-client

WebRTC (RTCPeerConnection, RTCDataChannel)

Backend (Signaling Server):

Node.js

Express

Socket.io

👉 https://github.com/islem05/p2p-file-sharing

---

## 📊 GitHub Stats

![GitHub stats](https://github-readme-stats.vercel.app/api?username=islem05\&show_icons=true\&theme=radical)

---

## 🔥 GitHub Streak

![GitHub Streak](https://streak-stats.demolab.com?user=islem05\&theme=radical)

---

## 📈 Top Languages

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=islem05\&layout=compact\&theme=radical)

---

## 🎯 Goals for 2026

* 🚀 Land a Data / Backend internship
* 🧠 Build advanced AI + data projects
* ⚙️ Master scalable backend systems

---

## 📫 Contact

* 💼 LinkedIn: https://www.linkedin.com/in/moh-ado/

---

